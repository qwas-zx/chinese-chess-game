...
### 三种对战模式
- **本地双人模式**：同屏轮流走棋，支持完整操作功能
- **人机对战模式**：基于 Minimax + Alpha-Beta 的增强型 AI，加入战术优先、移动排序、王安全评估和棋子位置表（PST），三档难度可调
- **在线联机模式**：房间系统、实时同步、WebSocket 通信

### 通用功能
- 完整的中国象棋规则实现（各棋子走法、将军/绝杀检测、和棋请求、摆棋模式）
- **走子高亮提示**：最近一步起点（黄色虚线边框）和终点（黄色实线边框+发光）的视觉标记
- **推演功能**：在浮动小窗口中基于当前局势自由走子推演，不影响真实棋盘
- 悔棋、认输、求和、棋盘翻转
- **复盘功能**：按步骤回放棋谱，支持跳转到任意步数
- **分析功能**：AI 推荐最佳走法，并给出局势评分
- **点评功能**：对每一步棋进行评价（精彩/合理/错失/较差）
- **棋谱导入/导出**：支持保存和加载完整棋谱
- **AI 思考指示器**：人机对战时显示 AI 正在计算

### 联机专属功能
- 实时聊天功能（WebSocket 实时通信）
- 三步同步协议（请求-确认-广播）
- 断线重连与状态追同步
- 房间管理（创建/加入/离开）

### 用户系统
- 用户注册与登录（基于 Flask Session + SQLite，密码经 werkzeug 哈希）
...
│           ├── game_logic.js   # 客户端象棋规则校验（推演用）
│           ├── deduce.js       # 推演功能（浮动小窗口，快照悔棋）
│           ├── ui.js           # 本地对战逻辑
│           ├── ai_battle.js    # 人机对战逻辑（AI 思考、复盘、分析）
│           ├── api.js          # HTTP 通信封装（所有 API 调用）
│           ├── auth_ui.js      # 登录状态检查 + 导航栏用户信息
│           └── online.js       # 联机对战逻辑（SocketIO 客户端、聊天）
│
├── templates/                  # HTML 模板
│   ├── index.html              # 本地对战页
...
└── tests/                      # 测试
    ...
    ├── test_ai_logic.py        # AI 引擎单元测试（走法选择、难度级别、战术判断）
    ├── test_ai_routes.py       # AI 复盘/点评接口回归测试
...
AI 引擎基于 **Minimax 算法 + Alpha-Beta 剪枝**，并加入了一系列实战化增强，纯 Python 实现，无需额外依赖。

- **搜索深度**：根据难度动态调整，使用迭代加深提升首着质量
- **评估函数**：棋子价值 + 棋子位置表（PST）+ 棋子活动性 + 王安全。评估函数刻意保持轻量，使搜索能到达更深层数
- **走法排序**：MVV-LVA（按吃子价值排序）+ 杀手走法 + 历史表 + 位置偏好，提升 Alpha-Beta 剪枝效率
- **搜索优化**：杀手走法、历史表、置换表、空着剪枝（Null Move Pruning）
- **战术优先**：发现明确赢利的吃子（吃无保护子或低成本吃高价值子）时直接短路，跳过完整搜索
- **走法生成**：递归搜索时使用伪合法走法（跳过王安全检查）以提升性能；根节点使用完整合法走法确保正确性
...
|------|----------|------|
| 简单 | 2 | 具有轻微随机性，适合新手上手 |
| 普通 | 4 | 平衡搜索与速度，具备一定挑战性 |
| 困难 | 5 | 更深的搜索和更强的局面判断，棋力更强 |
...
棋子位置表（PST）为每种棋子提供基于位置的加成/减成（如兵过河后加分、马在中央加分、王缩在九宫底线加分）。AI 始终执黑方。

---

## 推演功能说明

**推演**（Deduce）是一个辅助思考工具，让玩家在不影响真实棋盘的情况下，基于当前局势自由走子、预演后续变化。三种对战模式均已集成。
...
| 局部进行 | 推演完全在浏览器本地完成，不调用任何后端 API |
| 联机可见性 | 联机模式下推演仅自己可见，对手无感知 |
| 走法校验 | 基于客户端 `game_logic.js` 实现完整棋子走法规则校验 |
| 快照悔棋 | 每次走子前深拷贝棋盘+回合+历史，悔棋时恢复完整状态 |
...
### 分析功能
点击「分析」按钮，AI 会基于当前局势推荐最佳走法，并给出局势评分（正数为红方优势，负数为黑方优势）。

### 复盘功能
点击「复盘」按钮，打开复盘面板，支持：
- 查看每一步的详细记录
- 点击任意步骤跳转到对应棋盘状态
- 使用「上一步/下一步/首页/末页」按钮导航

### 点评功能
点击「点评」按钮，AI 会对每一步棋进行评价：

| 等级 | 含义 | 判定条件 |
|------|------|----------|
| 精彩 | 非常好的走法 | 得分提升 > 100 |
| 合理 | 正常走法 | 得分变化在正常范围 |
| 错失 | 错失更好的走法 | 与最佳走法差距 > 100 |
| 较差 | 不太好的走法 | 得分下降 > 50 |
...
| POST | `/api/import`      | 导入棋谱 |
| POST | `/api/analyze`     | 分析当前局势，推荐最佳走法 |
| POST | `/api/review`      | 点评历史走法 |
...
| POST | `/api/ai/draw`        | 请求求和 |
| POST | `/api/ai/analyze`     | 分析当前局势 |
| POST | `/api/ai/review`      | 点评历史走法 |
...
| POST | `/api/online/rooms/<id>/join` | 加入房间 |
| POST | `/api/online/rooms/<id>/leave`| 离开房间 |
| GET  | `/api/online/rooms/<id>`      | 获取房间信息 |
...
- 消息结构：`{msg_id, room_id, sender, type, seq, payload, timestamp}`
...
| `LOG_LEVEL` | 日志级别 | `INFO` |
| `LOG_FORMAT` | 日志格式（`human` / `structured`） | `human` |
...
```bash
python -m unittest tests.test_ai_routes
```

### 全量测试

```bash
python -m unittest tests.test_ai_logic tests.test_ai_routes
```
...
**A:** 请确保 CSS 文件已正确加载，刷新页面（Ctrl+F5）清除缓存。高亮样式包括 `.last-move-from`（起点黄色虚线）和 `.last-move-to`（终点黄色实线+发光）。
...
- **ui.js**：本地对战逻辑（状态管理、事件处理、复盘、分析）
- **ai_battle.js**：人机对战逻辑（AI 思考状态、难度切换、复盘、分析）
- **auth_ui.js**：登录状态检查、导航栏用户信息显示
- **online.js**：联机对战逻辑（SocketIO 客户端、三步协议、聊天）
...
- 所有同步操作必须遵循三步流程：请求-确认-广播
- 消息结构必须包含 msg_id, room_id, sender, type, seq, payload, timestamp
- seq 必须在房间内单调递增，从 0 开始
- 服务端必须在处理消息前验证发送者是否为当前回合玩家
- 服务端不应信任消息中的 sender 字段；用户身份必须从 WebSocket 连接读取
- 一个 user_id 只能在一个房间内；加入新房间会自动从旧房间踢出
- 房间 ID 必须大小写不敏感；所有房间 ID 比较必须使用大写转换
- 所有游戏 API 端点需要用户认证；未认证请求返回 401
...
### v1.3.0
- 新增推演功能：在浮动小窗口中基于当前局势自由走子推演
- 修复将帅照面（飞将）、绝杀、困毙等输赢判定问题
- 完善 CSS 样式（走子高亮、棋盘防抖动、响应式布局）

### v1.4.0
- 新增复盘功能：按步骤回放棋谱，支持跳转
- 新增分析功能：AI 推荐最佳走法，给出局势评分
- 新增点评功能：对每一步棋进行评价（精彩/合理/错失/较差）
- 联机模式新增聊天功能（WebSocket 实时通信）
- 本地模式新增棋谱导入/导出功能
- 修复过河兵横向移动问题
- 优化日志配置（DEBUG 级别可见性）

### v1.4.1
- 优化 AI 搜索与局面评估，增强战术判断与王安全意识
- 新增更稳定的走法排序与战术优先策略
- 修复复盘/点评接口对真实棋谱历史记录的兼容问题
- 增加 AI 回归测试，提升长期维护稳定性

### v1.4.2
- **修复 AI 评估函数关键缺陷**：
  - 修复 `_is_square_attacked` 无法识别友方棋子防守的 bug（此前导致 AI 把受保护棋子当作可免费吃子，跳过搜索直接走出败招）
  - 修复兵和王的棋子位置表（PST）方向反转问题（此前兵被惩罚推进、王被鼓励走出九宫）
  - 精简评估函数，移除冗余的机动性/威胁/防守计算（此前每叶节点 ~34000 次 `is_valid_move` 调用，5 秒仅搜 ~800 节点；修复后 ~4700 节点，搜索深度从 4 提升至 5）
  - 简化走法排序，移除每步 `deepcopy` + 全盘扫描，改用纯 MVV-LVA
- 统一代码注释为英语